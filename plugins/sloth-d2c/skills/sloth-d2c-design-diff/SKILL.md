...
description: "对比 Sloth/Figma 设计基准图与当前生成实现，并根据可证明的视觉差异修复 UI。首次生成完成后用于轻量截图自检；用户显式调用本 skill，或明确要求深度视觉 diff、还原设计、检查长页和响应式差异时执行完整分析；普通文案、交互或局部样式标注不要使用。"
---

# Sloth D2C 视觉对比

根据 `workspace`、`fileKey`、`nodeId`，以及可选的 `eventId`、`eventBrief` 和 `implementationUrl`，定位设计基准和实现页面，完成轻量自检或完整分析。

## 调用方式

- 首次实现完成后的自动验收使用轻量自检。用户显式调用本 skill，或明确要求深度视觉对比时使用完整分析。触发来源不明确时使用轻量自检。
...
4. 将当前实现截图保存到 `<session>/screenshots/implementation/design-diff[-<safeEventId>].png`。缺少 baseline 或 `implementationUrl` 时返回 blocked，不进入视觉审查，也不确认事件。实现页无法访问或截图失败时说明失败原因，不重复启动浏览器或截图。

## 截图

读取 baseline 尺寸，按同宽 viewport 使用一次性 Playwright、Puppeteer 或独立 headless 浏览器访问真实 `implementationUrl`，全页截取一张新的 candidate。每轮都截取当前实现，不复用修改前截图。

## 轻量自检

1. 直接查看 baseline 和 candidate，只做整页比较，不切片。
2. 只检查页面空白、核心资源缺失、明显结构错位、严重溢出和整体尺寸异常。
3. 没有明显问题时直接结束。发现明显问题时进行修正，并重新截图复验。
4. 复验后结束并记录剩余差异，不自动升级为完整分析。有代码修改时运行一个聚焦检查。

轻量自检不采集几何数据、不建立完整问题清单，也不追踪中低严重度差异。

## 完整分析

1. 先比较完整页面；只有长页、复杂页面或局部差异看不清时才按自然区域切片。
2. 在内部记录简洁问题清单：区域、证据、差异、严重度和状态。静态内容要求一致；动态字段只比较位置、格式、长度容错和状态契约。
3. 用户只要求审计时不修改实现。其它情况下修复可证明的高、中严重度差异，并重新截图复核。
4. 主要差异已经处理，或继续调整收益不高时结束并记录剩余差异；不要反复追逐低风险差异。
5. 有代码修改时运行一个能证明改动生效的聚焦检查。
